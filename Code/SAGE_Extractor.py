...
def M200_Disp_Cor(vd, rad):
    ...
    rad_lim = 0.369
    n2 = -1.591
    Ab = alpha * ((vd / vd_lim)**n1 - 1) if vd < vd_lim else 0
    Ac = beta * ((rad / rad_lim)**n2 - 1) if rad < rad_lim else 0
    A = 5/3 + Ab + Ac
...
def Fixes(data):
    data['Total_Stellar_Mass'] = np.log10(data['Total_Stellar_Mass'] * 1E10 / h)
    data['Total_Star_Formation_Rate'] = np.where(data['Total_Star_Formation_Rate'] > 0, np.log10(data['Total_Star_Formation_Rate']), np.nan)
    data['Mvir'] = np.log10(data['Mvir'] * 1E10 / h)
    data['Rvir'] = data['Rvir']  / h
    data['Central_Galaxy_Mvir'] = np.log10(data['Central_Galaxy_Mvir'] * 1E10 / h)
    data['vel'] = (data['Redshift_Observed']  * c.to(u.km/u.s).value) / (1 + data['Redshift_Observed'])
    data = data[data['Total_Stellar_Mass'] > 7.5].reset_index(drop=True)
    grouped = data.groupby('Central_Galaxy_ID') 
    Gdata = grouped.apply(lambda x: x.copy(), include_groups=True).reset_index(drop=True)    
    Gdata['Nm'] = Gdata.groupby('Central_Galaxy_ID')['Central_Galaxy_ID'].transform('size')
    Fdata = Gdata[Gdata['Nm'] < 3]
    Fdata = Fdata.apply(lambda x: x.copy()).reset_index(drop=True)
    Gdata = Gdata[Gdata['Nm'] >= 3]
    group_id_map = {cid: idx for idx, cid in enumerate(Gdata['Central_Galaxy_ID'].unique())}
    Gdata['group_id'] = Gdata['Central_Galaxy_ID'].map(group_id_map)
    data['sSFR'] = data['Total_Star_Formation_Rate'] - data['Total_Stellar_Mass']
    Gdata['sSFR'] = Gdata['Total_Star_Formation_Rate'] - Gdata['Total_Stellar_Mass']
    Fdata['sSFR'] = Fdata['Total_Star_Formation_Rate'] - Fdata['Total_Stellar_Mass']
    return data, Gdata, Fdata
...
def Group_Catalog(data):
    ...
    for group_id, group in data.groupby('group_id'):
        ...
        M_200_Disp_Correction = M200_Disp_Cor(vdisp_gap, radius)
...
def WriteTable(data, n, check):
    if check == 1:
        table = Table.from_pandas(data)
        table.write(f'/fred/oz004/wvankemp/Halo_Mass_New/Data/SAGE/data_{n}.fits', format='fits', overwrite=True)
    if check == 2:
        table = Table.from_pandas(data)
        table.write(f'/fred/oz004/wvankemp/Halo_Mass_New/Data/SAGE/data_group_{n}.fits', format='fits', overwrite=True)
    if check == 3:
        table = Table.from_pandas(data)
        table.write(f'/fred/oz004/wvankemp/Halo_Mass_New/Data/SAGE/data_field_{n}.fits', format='fits', overwrite=True)
    if check == 4:
        table = Table.from_pandas(data)
        table.write(f'/fred/oz004/wvankemp/Halo_Mass_New/Data/SAGE/data_group_catalog_{n}.fits', format='fits', overwrite=True)
        

for i in range(10):
    data = Table.read(f"/fred/oz004/wvankemp/Halo_Mass_New/Data/SAGE/tao.4692.{i}.fits")
    globals()[f"data{i}"] = data.to_pandas()


for i in range(10):
    data = globals()[f"data{i}"]
    data = data[data['SDSS_i_Apparent'] <= 19.2 + 5*np.log10(h/sim_h)].reset_index(drop=True)
    # data = data[data['Johnson_B_Apparent']  <= 19.56 + 5*np.log10(h/sim_h)].reset_index(drop=True)
    print(f"Processing data {i} with {len(data)} entries")
    data, gdata, fdata = Fixes(data)
    print(f"Data {i} processed with {len(data)} entries, {len(gdata)} groups, and {len(fdata)} fields")
    gdata, gcdata = Group_Catalog(gdata)
    print(f"Group catalog for data {i} created with {len(gcdata)} groups")


    WriteTable(data, i, 1)
    WriteTable(gdata, i, 2)
    WriteTable(fdata, i, 3)
    WriteTable(gcdata, i, 4)
    print(f"Data {i} done")